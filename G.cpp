...
class MaximumSegmentTree {
 private:
  ...
  Node<Type> Maximum(int permanent_left, int permanent_right, int current_left, int current_right, int position) {
    ...
    return Maximum(permanent_left, permanent_right, current_left, (current_left + current_right) / 2, LeftChild(position)) +
            Maximum(permanent_left, permanent_right, (current_left + current_right) / 2 + 1, current_right, RightChild(position));
  }
 ...
 public:
  explicit MaximumSegmentTree(const std::vector<Type> &input_array) {
    ...
    tree_.resize(2 * size_ - 1, Node(neutral_element_));
    for (int i = size_ - 1; i < real_size + size_ - 1; ++i) {
      tree_[i] = Node(input_array[i - size_ + 1]);
    }
...
int main() {
  ...
  for(int i = 0, buffer; i < amount; ++i) {
    std::cin >> buffer;
    input_array.push_back(buffer);
  }
  MaximumSegmentTree seg_tree (input_array);
  std::cout << seg_tree.Maximum(1, 3).second << '\n';
  return 0;
}