...
int main() {
  ...
  int64_t left_argument = 0;
  int64_t right_argument = 0;
  for(int i = 0; i < amount; ++i) {
    cin >> request_type;
    if(request_type == '?') {
      cin >> left_argument >> right_argument;
      cout << treap.Sum(left_argument, right_argument) << '\n';
    } else if(request_type == '+') {
      cin >> left_argument;
      treap.Insert(left_argument);
    }
  }
  cin.close();
  cout.close();
